...
> 适用于「一个独立域名对一台 NextAPI 实例」的典型部署，例如 `https://nextapi.example.com/`。
...
server {
    listen 80;
    listen [::]:80;
    server_name nextapi.example.com;
...
server {
    ...
    ssl_certificate     /etc/nginx/ssl/nextapi.example.com.cer;
    ssl_certificate_key /etc/nginx/ssl/nextapi.example.com.key;
...
# 健康与就绪（经域名走代理链路）
curl -i https://nextapi.example.com/healthz
curl -i https://nextapi.example.com/readyz

# 首页（SPA）可达
curl -I https://nextapi.example.com/
...
curl -N https://nextapi.example.com/v1/chat/completions \
  -H "Authorization: Bearer sk-网关Key" -H "Content-Type: application/json" \
  -d '{"model":"gpt-4o","stream":true,"messages":[{"role":"user","content":"hi"}]}'
```
